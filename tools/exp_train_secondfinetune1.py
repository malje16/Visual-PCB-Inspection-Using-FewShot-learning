...
def My_train_aug(cfg):
    ...
    augs.append(T.RandomExtent(scale_range=[0.5, 1.0], shift_range=[0.8, 0.8]))
...
class Trainer(DefaultTrainer):
    ...
    def build_model(cls, cfg):
        ...
        #------------------------------------------------------------------------------------------
        # attempting to unfreeze a few layers. 
        backbone_counter = 0
        box_head_counter = 0
        proposal_generator_counter = 0

        backbone_unfreeze_point = 112
        box_head_unfreeze_point = 3 
        proposal_generator_unfreeze_point = 5

        for p in model.backbone.parameters(): # Count 120
            #print(p)
            backbone_counter += 1
            if backbone_counter > backbone_unfreeze_point: 
        ...
        for p in model.roi_heads.box_head.parameters(): # Count 4
            #print(p)
            box_head_counter += 1
            if box_head_counter > box_head_unfreeze_point: 
                p.requires_grad = True
        ...
        for p in model.proposal_generator.parameters(): # Count 6
            ...
            if proposal_generator_counter > proposal_generator_unfreeze_point: 
                p.requires_grad = True
